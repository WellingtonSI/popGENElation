...
$(document).on('click','.btnCalcular', function(){
    ...
    if(u==='' || Number(u)===0){
        swal("Atenção!", "Preencha corretamente o campo de Taxa de Mutação de “A” p/ “a” (µ)",'error');}
    else if(v==='' || Number(v)===0){
        swal("Atenção!", "Preencha corretamente o campo de Taxa de Mutação de “a” p/ “A” (v)",'error');}
    else{
        ...
        if(Number(valorTotal)===1){
           ...
           $.ajax({
                ...
                error: function(error) {
                    swal("Atenção!", "Não foi possível realizar os cálculos, tente novamente mais tarde", "error");
                },
            });
       
        }else{
            if(valorTotal>1)
                swal("Atenção!", "As somas das frequências (p+q) está maior que 1", "error");
            else
                swal("Atenção!", "As somas das frequências (p+q) está menor que 1", "error");
        }
    }
